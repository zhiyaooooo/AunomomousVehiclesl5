...
// C/C++ source code generated on : Thu Oct 31 00:51:02 2024
//
...
void l5_step(void)
{
...

  {
    ...
    //   Constant: '<Root>/alpha1'
    //   Constant: '<Root>/desired time gap1'
    //   Integrator: '<Root>/Integrator1'

    time_gap = l5_B.In1_o.Data / l5_B.In1.Data;
    a = (l5_B.In1_o.Data - l5_P.desiredtimegap1_Value * l5_B.In1.Data) *
      l5_P.alpha1_Value - 1.5 * l5_X.Integrator1_CSTATE;
    if ((time_gap < l5_P.desiredtimegap1_Value + 0.5) && (time_gap >
         l5_P.desiredtimegap1_Value - 0.5)) {
      a *= 0.1;
    }
    ...
    time_gap = l5_M->Timing.t[0];
    if ((l5_DW.TimeStampA >= time_gap) && (l5_DW.TimeStampB >= time_gap)) {
      time_gap = 0.0;
    } else {
      lastTime = l5_DW.TimeStampA;
      lastU = &l5_DW.LastUAtTimeA;
      if (l5_DW.TimeStampA < l5_DW.TimeStampB) {
        if (l5_DW.TimeStampB < time_gap) {
          lastTime = l5_DW.TimeStampB;
          lastU = &l5_DW.LastUAtTimeB;
        }
      } else if (l5_DW.TimeStampA >= time_gap) {
        lastTime = l5_DW.TimeStampB;
        lastU = &l5_DW.LastUAtTimeB;
      }

      time_gap = (l5_B.In1.Data - *lastU) / (time_gap - lastTime);
    }
  ...
  if (rtmIsMajorTimeStep(l5_M)) {
    ...
    if (l5_DW.TimeStampA == (rtInf)) {
      l5_DW.TimeStampA = l5_M->Timing.t[0];
      lastU = &l5_DW.LastUAtTimeA;
    } else if (l5_DW.TimeStampB == (rtInf)) {
      l5_DW.TimeStampB = l5_M->Timing.t[0];
      lastU = &l5_DW.LastUAtTimeB;
    } else if (l5_DW.TimeStampA < l5_DW.TimeStampB) {
      l5_DW.TimeStampA = l5_M->Timing.t[0];
      lastU = &l5_DW.LastUAtTimeA;
    } else {
      l5_DW.TimeStampB = l5_M->Timing.t[0];
...
void l5_derivatives(void)
{
  ...
  _rtXdot->Integrator1_CSTATE = l5_B.err;
}
...
void l5_initialize(void)
{
...

  {
    ...
    static const char_T b_zeroDelimTopic_1[15] = "/ego_car/vel_x";
    ...
    l5_DW.TimeStampA = (rtInf);
    l5_DW.TimeStampB = (rtInf);
    ...
    Sub_l5_34.createSubscriber(&b_zeroDelimTopic[0], 1);
    ...
    Sub_l5_31.createSubscriber(&b_zeroDelimTopic_0[0], 1);
    ...
    Pub_l5_18.createPublisher(&b_zeroDelimTopic_0[0], 1);
